...
const categoryStructure = [
  ...
  {
    name: "Structures",
    subCategories: ["All Structures", "Heavy-Duty I-Beams", "Purlins"],
  },
  {
    name: "Accessories",
...
export default function ProductsPage() {
  ...
  // FIX:
  // The refs are nullable because ref.current is null
  // before the DOM element is mounted.
  ...
  const handleMouseMove = (
    ...
    const startX = parseFloat(
      ref.current.dataset.startX || "0"
    );

    const scrollLeft = parseFloat(
      ref.current.dataset.scrollLeft || "0"
    );
      ...
      <section className="py-20 md:py-32 px-6 md:px-16 lg:px-24 max-w-[1400px] mx-auto">
        ...
        <div className="flex flex-col gap-8 pb-8 border-b border-zinc-200 mb-8 select-none">
          ...
          <motion.div
            ...
            ref={mainScrollRef}
            onMouseDown={(e) =>
              handleMouseDown(e, mainScrollRef)
            }
            onMouseLeave={() =>
              handleMouseLeaveOrUp(mainScrollRef)
            }
            onMouseUp={() =>
              handleMouseLeaveOrUp(mainScrollRef)
            }
            onMouseMove={(e) =>
              handleMouseMove(e, mainScrollRef)
            }
          ...
          {hasSubCategories && (
            ...
            >
              <div
                ref={subScrollRef}
                onMouseDown={(e) =>
                  handleMouseDown(e, subScrollRef)
                }
                onMouseLeave={() =>
                  handleMouseLeaveOrUp(subScrollRef)
                }
                onMouseUp={() =>
                  handleMouseLeaveOrUp(subScrollRef)
                }
                onMouseMove={(e) =>
                  handleMouseMove(e, subScrollRef)
                }
            ...
            {filteredProducts.length > 0 ? (
              ...
              >
                ...
                <p className="text-[14px] md:text-[15px] text-zinc-500 font-light max-w-sm md:max-w-md leading-relaxed">
                  We are currently updating our inventory for
